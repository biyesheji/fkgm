...
class AdminnewsController extends AdminController
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['News']))
		{
            ...
            // 上传视频
             if($_FILES && !empty($_FILES['News']['type']['video'])){
                ...
                if(is_array($video_icon_save_name) && ($video_icon_save_name['code'] == 0)){
                    ...
                    $is_img_moved = Tool::moveVideoToTargetPath($video_icon_save_name, $tmp_name, 500,600);
                    if($is_img_moved){
            ...
            if ($model->validate()) {
                if($model->save())
                    Service::addLog('新建新闻，内容是：'.json_encode($model->attributes));
    ...
    public function actionUpdate($id)
    {
        ...
        if(isset($_POST['News']))
        {
            ...
            $model->tags = $_POST['News']['tags'];
            if($model->update())
                Service::addLog('修改新闻信息，由：'.json_encode($model_info->attributes).' 修改为：'.json_encode($model->attributes));
                $this->redirect(array('view','id'=>$model->id));
        }
	...
	public function actionDelete($id)
	{
        ...
        Service::addLog('删除新闻，内容是：'.json_encode($model_info->attributes));
        $model_info->delete();

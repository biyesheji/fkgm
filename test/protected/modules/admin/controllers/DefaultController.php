...
class DefaultController extends Controller
{
    ...
    public function accessRules(){
            return array(
                array(
                    'allow',
                    'actions' => array('index','login','captcha','logout'),
                    'users' => array('@'),
                    ),
                array(
                    'allow',
        ...
        if ($type) {
            if ($type === 'index_boom_info') {
                ...
                if (Yii::app()->request->isPostRequest) {
                    Service::addLog(Yii::app()->user->id.'修改页面底部变量信息，内容是：'.json_encode($_POST));
                ...
                $info = Service::GetVariable($type);
                 ...
                 if (Yii::app()->request->isPostRequest) {
                    Service::addLog(Yii::app()->user->id.'修改联系我们变量信息，内容是：'.json_encode($_POST));
                    $data = array();
                ...
                if (Yii::app()->request->isPostRequest) {
                    $data = array();
                    Service::addLog(Yii::app()->user->id.'修改关于我们变量信息，内容是：'.json_encode($_POST));
                    $data['top'] = Yii::app()->request->getParam('top');
                ...
                if (Yii::app()->request->isPostRequest) {
                    Service::addLog(Yii::app()->user->id.'修改紧急统计变量信息，内容是：'.json_encode($_POST));
                    $data = array();
                ...
                if (Yii::app()->request->isPostRequest) {
                    $data = array();
                    Service::addLog(Yii::app()->user->id.'修改邮件配置变量信息，内容是：'.json_encode($_POST));
                ...
                if (Yii::app()->request->isPostRequest) {
                    $data = array();
                    Service::addLog(Yii::app()->user->id.'修改统计代码变量信息，内容是：'.json_encode($_POST));
                ...
                $info = Service::GetVariable($type);
                if (Yii::app()->request->isPostRequest) {
                    Service::addLog(Yii::app()->user->id.'修改SEO变量变量信息，内容是：'.json_encode($_POST));
                    $data = array();
                ...
                $info = Service::GetVariable($type);
                if (Yii::app()->request->isPostRequest) {
                    Service::addLog(Yii::app()->user->id.'修改分享变量信息，内容是：'.json_encode($_POST));
                    $data = array();
                ...
                return $this->render('fenxiang',array('info' => $info));
                 ...
                 $info = Service::GetVariable($type);
                if (Yii::app()->request->isPostRequest) {
                    Service::addLog(Yii::app()->user->id.'修改qq变量信息，内容是：'.json_encode($_POST));
                    $data = array();
    ...
    public function actionLogin(){
        ...
        if (Yii::app()->request->isPostRequest) {
            ...
            if ($login) {
                Service::addLog(Yii::app()->user->id.' 登陆成功');
                return $this->redirect('/admin/default/index');
            }
        }
    ...
    public function actionlogout(){
        Service::addLog(Yii::app()->user->id.' 退出成功');
        Yii::app()->user->logout();

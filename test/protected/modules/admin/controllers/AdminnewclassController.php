...
class AdminnewclassController extends AdminController
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Newclass']))
		{
			...
			$model->time = date('Y-m-d H:i:s',time());
	 		if($model->save())
	 			Service::addLog(Yii::app()->user->id.'新建新闻类别，内容是：'.json_encode($model->attributes));
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Newclass']))
		{
			...
			$model->time = date('Y-m-d H:i:s',time());
			if($model->save())
	 			Service::addLog(Yii::app()->user->id.'修改新闻类别，内容是：'.json_encode($model_info).' 修改为:'.json_encode($model->attributes));
				$this->redirect(array('view','id'=>$model->id));
		}
	...
	public function actionDelete($id)
	{
		...
		$model_info->delete();
	 	Service::addLog(Yii::app()->user->id.'删除新闻类别，内容是：'.json_encode($model_info->attributes));

...
class AdminlinkController extends AdminController
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Link']))
		{
			...
			if($model->save())
                Service::addLog(Yii::app()->user->id.'新建友情链接，内容是：'.json_encode($model->attributes));
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Link']))
		{
			...
			if($model->save())
                Service::addLog(Yii::app()->user->id.'修改友情链接，有：'.json_encode($model_info).' 修改为：'. json_encode( $model->attributes));
				$this->redirect(array('view','id'=>$model->id));
		}
	...
	public function actionDelete($id)
	{
        ...
        Service::addLog(Yii::app()->user->id.'删除友情链接，内容是：'.json_encode($model_info->attributes));
